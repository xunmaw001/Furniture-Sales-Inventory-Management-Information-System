						...
						<div class="widget-area-2 lochana-box-shadow">
							...
							<form id="addOrUpdateForm">
								<div class="form-row">
									...
									<div class="form-group col-md-6">
										<label>退货备注</label>
										<textarea  name="tuihuobeizhu" class="form-control" id="tuihuobeizhu" rows="3"></textarea>
									</div>
	...
	<script>
				...
				if(crossrefid && crossuserid) {
					...
					http("xiaoshoutuihuo/page","GET",{
						...
						if(res.data.total >= crossoptnum){
							...
							httpJson("xiaoshoutuihuo/"+urlParam,"POST",data,(res)=>{
								if(res.code == 0){
		...
		function getDetails() {
			...
			if(id != null && id != "" && id != "null"){
				updateId = id;
				window.sessionStorage.removeItem('id');
				http("xiaoshoutuihuo/info/" + id,"GET",{},(res)=>{
					if(res.code == 0){
						ruleForm = res.data
						// 是/否下拉框回显
						setSelectOption();
		...
		function crossTableUpdate(){
			...
			if(flag){
				httpJson(crossTableName + "/update","POST",{
					id: crossTableId,
					tuihuojine: Number(crossRuleForm.tuihuojine) + Number($('#tuihuojine').val()),
				},(res)=>{
					if(res.code == 0){
						console.log('跨表更新成功');
						return true;
